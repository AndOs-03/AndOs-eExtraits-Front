...
export default function HeaderCentreActifZone() {

  const [isOpen, setIsOpen] = useState(false);
  const [selectedCenter, setSelectedCenter] = useState<Centre>({
    id: 0,
    nom: "-- Sélectionnez --"
  });
  const [centres, setCentres] = useState<Centre[]>([]);

  const listerCentres = async () => {
    try {
      const centre = recupererCentreActif();
      if (centre !== null) {
        setSelectedCenter({id: centre.id, nom: centre.nom});
      }
      ...
      if (!("message" in reponse)) {
        setCentres(reponse);
        if (reponse.length < 1) {
          setSelectedCenter({id: 0, nom: "-- Sélectionnez --"});
          setCentres([{id: 0, nom: "-- Sélectionnez --"}]);
        }
      } else {
  ...
  return (
      <div className="relative font-sans text-gray-800">
        <span className="font-bold text-gray-800 dark:text-white/90 mr-2">Centre actif :</span>
        <div className="inline-block relative">
          ...
          >
            <span className="text-gray-500 dark:text-gray-400 font-semibold">{selectedCenter.nom}</span>
            <svg
